...
/**
 * Fill in customer details that Stripe has but we never stored.
 *
 * Orders taken before the webhook read `collected_information.shipping_details`
 * saved whatever was in `customer_details` — which for a shipping-only checkout
 * is usually nothing, so the admin shows a bare email where the name belongs.
 * The session still holds it, so this reads it back.
 *
 * Only fills blanks: anything already recorded, or edited by hand, is left
 * alone. Skips manual and imported orders, which have no Stripe session.
 */
export async function backfillFromStripe(): Promise<{
  ok: boolean;
  message: string;
}> {
  if (!isStripeConfigured()) {
    return { ok: false, message: "Stripe isn't configured." };
  }
  const prisma = getPrisma();
  const candidates = await prisma.order.findMany({
    where: {
      source: "web",
      OR: [{ customerName: null }, { phone: null }, { shipping: { equals: Prisma.DbNull } }],
    },
    orderBy: { createdAt: "desc" },
    take: 200,
  });
  const withSession = candidates.filter((o) =>
    o.stripeSessionId.startsWith("cs_"),
  );
  if (withSession.length === 0) {
    return { ok: true, message: "Nothing to fill in — every order has its details." };
  }
  ...
  for (const order of withSession) {
    try {
      ...
      if (!order.customerName && name) data.customerName = name;
      if (!order.phone && details?.phone) data.phone = details.phone;
      ...
      // A session Stripe has since expired from its API is the common case;
  ...
  if (filled === 0) {
    return {
      ...
      message: failures.length
        ? `Stripe had nothing to add. First error: ${failures[0]}`
        : "Stripe had no extra details for those orders.",
    };
  }
  return {
    ok: true,
    message: `Filled in ${filled} order${filled === 1 ? "" : "s"} from Stripe.${
      failures.length ? ` ${failures.length} couldn't be read.` : ""
    }`,
  };
}
